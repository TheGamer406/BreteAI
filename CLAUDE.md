...
- **IA local (Fase 2): LM Studio en vez de Ollama en esta PC.** El usuario ya tenía LM Studio instalado (`lmstudio-bin` por pacman, `/opt/lm-studio/lm-studio.AppImage`) — se usa ese en dev en lugar de instalar Ollama de cero. El cliente (`app/ai/client.py`) habla la **API OpenAI-compatible** (`/v1/chat/completions`, `/v1/embeddings`, `/v1/models`) que exponen ambos casi igual, así que el mismo código sirve para los dos; solo cambia `OLLAMA_HOST`/`OLLAMA_MODEL` en `.env`. Ollama sigue siendo el runtime documentado para el server (`requirements.md` §5.2, `docker-compose.yml` de Infra ya tiene el servicio).
- **Levantar LM Studio headless** (sin ventana, para llamarlo desde scripts/tests): `nohup /opt/lm-studio/lm-studio.AppImage --headless &` (deja quieto ~5-8s a que arranque), después `~/.lmstudio/bin/lms load "qwen2.5-7b-instruct" -y` y `~/.lmstudio/bin/lms load "text-embedding-nomic-embed-text-v1.5" -y`. Server queda en `localhost:1234`. Ojo: **LM Studio exige `response_format` tipo `json_schema` o `text`** (no acepta el `json_object` "flojo" de OpenAI/Ollama) — `client.py` ya maneja el fallback.
